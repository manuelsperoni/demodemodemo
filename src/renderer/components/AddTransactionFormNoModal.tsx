...
export default function AddTransactionFormNoModal() {
          ...
          <Flex flex="1 1 auto" wrap="wrap" gap={2}>
            {state.directions.map((el) => (
              <Center
                key={el.id}
                padding={2}
                bg={el.id === direction?.id ? 'brand.accent' : 'transparent'}
                borderColor="brand.400"
                borderWidth={1}
                _hover={{ bg: 'brand.400' }}
                borderRadius={5}
              ...
              >
                <Text
                  color={el.id === direction?.id ? 'brand.100' : 'brand.300'}
          ...
          <Flex flex="1 1 auto" wrap="wrap" gap={2}>
            {state.categories.map((el) => (
              <Center
                key={el.id}
                padding={2}
                bg={el.id === category?.id ? 'brand.accent' : 'transparent'}
                borderColor="brand.400"
                borderWidth={1}
                _hover={{ bg: 'brand.400' }}
                borderRadius={5}
              ...
              >
                <Text
                  color={el.id === category?.id ? 'brand.100' : 'brand.300'}
                  _hover={{ color: 'brand.100' }}
                >
                  {el.description}
                </Text>
              ...
              {subcategoryFromCategory(state.categories, category.id).map(
                (el) => (
                  <Center
                    key={el.id}
                    bg={
                      el.id === subcategory?.id ? 'brand.accent' : 'transparent'
                    }
                    padding={2}
                    borderColor="brand.400"
                    borderWidth={1}
                  ...
                  >
                    <Text
                      color={
                        el.id === subcategory?.id ? 'brand.100' : 'brand.300'
                      }
                      _hover={{ color: 'brand.100' }}
                    >
                      {el.description}
